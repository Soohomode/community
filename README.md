...
### 성능 측정 결과 (k6 부하테스트, 10 VUs / 30s)

| 지표 | 캐시 적용 전 (Before) | 캐시 적용 후 (After) | 개선율 |
|------|----------------------|---------------------|--------|
| 평균 응답시간 | 8.98ms | 3.94ms | **56% 감소** |
| p(95) 응답시간 | 12.71ms | 5.80ms | **54% 감소** |
| 최대 응답시간 | 127.66ms | 7.21ms | **94% 감소** |
| 초당 처리량 | 19.6 req/s | 19.8 req/s | 동일 |
